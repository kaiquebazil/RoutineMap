...
import {
  ChevronLeft, ChevronRight, MessageSquareText, Trophy, AlertTriangle, Lightbulb, Check
} from 'lucide-react';
...
export function WeeklyReflection() {
  ...
  const loadReflection = useCallback((ws: string) => {
    const data = storage.getWeeklyReflection(ws);
  ...
  useEffect(() => {
    if (weekStart) {
      loadReflection(weekStart.toISOString().split('T')[0]);
    }
  }, [weekStart, loadReflection]);

  const autoSave = useCallback((field: 'threeWords' | 'proudMoments' | 'struggles' | 'learned', value: string) => {
    if (!weekStart) return;
    ...
    saveTimeout.current = setTimeout(() => {
      ...
      const payload = {
        ...
        learned,
        [field]: value,
      } as storage.WeeklyReflection;
      storage.saveWeeklyReflection(payload);
      setSaved(true);
